...
from psy_agents_noaug.deployment.health_validator import HealthValidator, MetricsValidator
from psy_agents_noaug.deployment.rollback import AutoRollbackMonitor, RollbackManager
...
class DeploymentOrchestrator:
    ...
    def deploy(self, config: DeploymentConfig, previous_version: str | None = None) -> DeploymentRecord:
        """Execute deployment.
        ...
        try:
            ...
            if not health_validator.validate_deployment(config.target.get_endpoint_url()):
            ...
            if config.rollback_on_error_rate:
                ...
                record.add_log(f"Monitoring error rate (threshold: {config.error_rate_threshold:.2%})")
            ...
            record.add_log(f"Deployment completed: {config.model_name} v{config.model_version}")
    ...
    def rollback_deployment(self, deployment_id: str, reason: str = "Manual rollback") -> bool:
    ...
    def list_deployments(self, status: DeploymentStatus | None = None) -> list[DeploymentRecord]:
    ...
    def get_deployment_stats(self) -> dict[str, Any]:
        ...
        completed = len([d for d in self.deployments.values() if d.status == DeploymentStatus.COMPLETED])
        failed = len([d for d in self.deployments.values() if d.status == DeploymentStatus.FAILED])
        rolled_back = len([d for d in self.deployments.values() if d.status == DeploymentStatus.ROLLED_BACK])
        ...
        avg_duration = (
            sum(d.get_duration_seconds() for d in completed_deployments) / len(completed_deployments)
