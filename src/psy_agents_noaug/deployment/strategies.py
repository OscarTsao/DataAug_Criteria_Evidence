...
class CanaryDeploymentStrategy(DeploymentStrategy):
    ...
    def execute(self, record: DeploymentRecord) -> bool:
        ...
        try:
            ...
            record.add_log(f"Deploying canary with {self.config.canary_traffic_percent}% traffic")
            if not self._simulate_deployment(record, "deploy_canary"):
                return False

            # Route small percentage of traffic to canary
            record.add_log(f"Routing {self.config.canary_traffic_percent}% traffic to canary")
            ...
            for traffic_percent in traffic_steps:
                ...
                if not self._simulate_deployment(record, f"increase_traffic_{traffic_percent}"):
                    return False
...
def get_strategy(config: DeploymentConfig) -> DeploymentStrategy:
    ...
    """
    from psy_agents_noaug.deployment.deployment_config import DeploymentStrategy as StrategyEnum
