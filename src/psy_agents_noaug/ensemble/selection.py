...
class DiversitySelector:
    ...
    def _select_hybrid(
        ...
        configs = [entry[config_key] for entry in pareto_front]
        scores = np.array(
            [entry.get(score_key, entry.get("f1_macro_mean", 0.0)) for entry in pareto_front]
        )
        ...
        for iteration in range(self.n_models):
            if iteration == 0:
                ...
                for cand in candidates:
                    ...
                    hybrid = (1 - self.diversity_weight) * perf_score + self.diversity_weight * div_score
...
def compute_config_distance(
    ...
    for key in all_keys:
        ...
        if val1 is None or val2 is None:
            ...
            differences += abs(val1_norm - val2_norm) / (abs(val1_norm) + abs(val2_norm) + 1e-8)
        elif val1 != val2:
