    ...
    def check_health(self, study: optuna.Study) -> HealthStatus:
        ...
        if finished > 0:
            ...
            if failure_rate > self.max_failure_rate:
                ...
                status.add_warning(
                    f"Elevated failure rate: {failure_rate*100:.1f}%"
                )

        # Check progress (completed trials)
        completed_trials = [t for t in trials if t.state == TrialState.COMPLETE]
        if completed_trials:
            latest = max(completed_trials, key=lambda t: t.datetime_complete or datetime.min)
            if latest.datetime_complete:
                time_since_completion = datetime.now() - latest.datetime_complete.replace(tzinfo=None)
